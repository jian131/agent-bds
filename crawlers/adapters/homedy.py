...
class HomedyAdapter(BasePlatformAdapter):
    """Adapter for homedy.com"""

    platform_id = "homedy"
    platform_name = "Homedy.com"
    ...
    def parse_search_results(self, html: str) -> list[UnifiedListing]:
        ...
        cards = soup.select(".product-item, .listing-item, article.property")
    ...
    def _parse_card(self, card: Tag) -> Optional[UnifiedListing]:
        """Parse listing card"""
        link = card.select_one("a.title, h3 a, a[href*='/ban-']")
        if not link:
            return None
        ...
        area_elem = card.select_one(".area, .product-area")
    ...
    def parse_detail_page(self, html: str, listing_id: str) -> UnifiedListing:
        ...
        return UnifiedListing(
            ...
            url=f"{self.base_url}/ban-nha-dat-i{listing_id}",
            contact_phone=phones[0] if phones else None,
        )

    def get_detail_url(self, listing_id: str) -> str:
        return f"{self.base_url}/ban-nha-dat-i{listing_id}"


PlatformRegistry.register(HomedyAdapter)
