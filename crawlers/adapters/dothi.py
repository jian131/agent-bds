"""
Dothi.net Platform Adapter

Real estate platform focused on urban properties.
"""
...
class DothiAdapter(BasePlatformAdapter):
    """Adapter for dothi.net"""

    platform_id = "dothi"
    platform_name = "Dothi.net"
    ...
    def parse_search_results(self, html: str) -> list[UnifiedListing]:
        ...
        cards = soup.select(".vip-item, .normal-item, .listing-item, article")
    ...
    def _parse_card(self, card: Tag) -> Optional[UnifiedListing]:
        """Parse card"""
        link = card.select_one("a[href*='/ban-'], a.title")
        if not link:
            return None
        ...
        area_elem = card.select_one(".area, .item-area")
    ...
    def parse_detail_page(self, html: str, listing_id: str) -> UnifiedListing:
        ...
        desc_elem = soup.select_one(".description, .detail-content")
    ...
    def get_detail_url(self, listing_id: str) -> str:
        return f"{self.base_url}/ban-nha-dat-{listing_id}.html"


PlatformRegistry.register(DothiAdapter)
